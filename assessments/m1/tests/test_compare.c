...
#include <assert.h>
#include "../stats.h"
void test_compare_asc(unsigned char* a , unsigned char* b, int expected){
    printf("compare_asc(%d, %d) expected %d\n", *a, *b, expected);
    int actual = compare_asc(a, b);
    assert(actual == expected);
}

void test_compare_desc(unsigned char* a , unsigned char* b, int expected){
